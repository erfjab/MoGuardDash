...
<template>
  <Popover>
    ...
    <PopoverContent class="w-[240px] p-0" align="start">
      <Command>
        <CommandInput :placeholder="t('subscriptions.searchOrder')" />
        <CommandList>
          ...
          <template v-if="selectedValue && props.allowClear !== false">
            ...
            <CommandGroup>
              <CommandItem
                :value="{ label: t('subscriptions.clearOrder') }"
                class="justify-center text-center"
                @select="clearOrder"
              >
                {{ t('subscriptions.clearOrder') }}
              </CommandItem>
            </CommandGroup>
          </template>
        </CommandList>
...
<script lang="ts">
// Helper exported in SFC to keep template types clean
export default {
  methods: {
