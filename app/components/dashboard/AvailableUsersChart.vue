...
const { data: statusStats, status } = useAsyncData(
  'subscription-status-stats-available',
...
const available = computed(() => statsData.value?.available ?? 0)
const unavailable = computed(() => {
  const apiUnavailable = statsData.value?.unavailable
  if (typeof apiUnavailable === 'number') return apiUnavailable
  return Math.max(total.value - available.value, 0)
})
...
const accentStart = 'var(--chart-3, #8b5cf6)'
const accentEnd = 'var(--chart-5, #6d28d9)'
const accentMuted = 'var(--chart-2, rgba(139, 92, 246, 0.15))'
const trackColor = 'var(--chart-track, rgba(15, 23, 42, 0.15))'
const tickColor = 'var(--chart-track-muted, rgba(15, 23, 42, 0.08))'
const gradientId = 'availabilityGradient'
  ...
  <Card class="flex flex-col">
    <CardHeader class="pb-2">
      <CardTitle class="flex items-center gap-2 text-base font-semibold">
        ...
        {{ $t('dashboard.systemAvailability') }}
      </CardTitle>
      <p class="text-xs text-muted-foreground">{{ $t('dashboard.systemAvailabilityDescription') }}</p>
    </CardHeader>
            ...
            <div class="flex flex-wrap items-center justify-center gap-6 text-center text-[9px] font-medium uppercase tracking-[0.25em] text-muted-foreground/65">
              <div class="flex flex-col gap-1">
                ...
                <span class="text-xs font-semibold tracking-normal text-foreground/65">
                  <AnimatedNumber :value="available" :duration="1500" />
                </span>
              </div>
              <div class="flex flex-col gap-1">
                <span>{{ $t('dashboard.unavailable') }}</span>
                <span class="text-xs font-semibold tracking-normal text-foreground/65">
                  <AnimatedNumber :value="unavailable" :duration="1500" />
                </span>
              </div>
            </div>
          </div>
